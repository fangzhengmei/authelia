...
**重要更正**：Elevation 状态**不直接存储在 Cookie 中**，而是通过以下分层存储架构：

```
浏览器 Cookie
    ↓ (只存储 Session ID)
服务端会话存储（Memory / Redis）
    ↓ (存储完整 UserSession JSON)
...
1. **Cookie 层**：
   - Cookie 中仅存储加密的 Session ID（由 `fasthttp/session` 管理）
   - Cookie 配置：`session.cookies[].name`（默认 `authelia_session`）
   - Session ID 生成：32 随机字节，经过字符集编码

2. **服务端会话存储层**（`internal/session/provider_config.go`）：
   - **Memory 模式**（默认）：会话数据存储在服务端内存中
   - **Redis 模式**：会话数据存储在 Redis 中，通过 `EncryptingSerializer` 加密
     - 加密算法：AES-GCM 256位密钥（通过 SHA-256 派生自 `session.secret`）
     - 序列化：MessagePack + AES-GCM 加密

3. **UserSession 序列化流程**（`internal/session/session.go`）：
   - `GetSession()`：从服务端存储取出 JSON 反序列化为 Go 结构体
   - `SaveSession()`：将 UserSession 序列化为 JSON 存入服务端存储
...
#### 2.3.3 撤销操作**（`handler_session_elevation.go:433`）：
```go
// 调用存储层标记为 revoked
...
| 失效机制 | 触发时机 | 检查位置 | 失效范围 |
|-----------|----------|----------|----------|
| **时间过期 | OTC 过期 | POST→PUT | OTC 失效 |
| **时间过期 | Elevation 过期 | GET / RequireElevated | Elevation 会话失效 |
| **IP 变化 | 访问受保护端点 | RequireElevated | Elevation 会话失效 |
| **主动撤销 | 用户取消/邮件链接 | DELETE /elevation | OTC 标记为 revoked |
| **已消费 | OTC 使用后 | PUT /elevation | OTC 标记为 consumed |
| **会话销毁 | 用户登出 | DestroySession | 整个 UserSession 销毁 |
...
| 会话存储 | `internal/session/session.go` | 29-78 (GetSession/SaveSession) |
