# Authelia TOTP 注册与验证机制分析报告（修订版 R5）
...
**速率限制机制**（关键修正）：
- 速率限制基于 IP 的令牌桶算法
- **只要请求到达了 TOTP endpoint，速率限制计数就会 +1**
- **与后续验证结果（成功/失败/重放）完全无关**
- 速率限制在中间件层执行，早于 doMark 记录
- 代码证据：`internal/middlewares/rate_limiting.go:60-94`
- 中间件顺序证据：`internal/middlewares/bridge.go:59-74`
...
| **doMark 与限流关系** | ❌ 无关 | ❌ 无关 | ❌ 无关 |
| | | | |
| **监管记录写入** | ✅ 写入（失败记录） | ❌ 不写入 | ✅ 写入（成功记录） |
| **监管表** | authentication_log | - | authentication_log |
| **认证类型** | AuthTypeTOTP（失败） | - | AuthTypeTOTP（成功） |
| | | | |
| **速率限制计数** | ✅ 增加（请求已到达 endpoint） | ✅ 增加（请求已到达 endpoint） | ✅ 增加（请求已到达 endpoint） |
| **速率限制与 doMark 关系** | ❌ 完全无关 | ❌ 完全无关 | ❌ 完全无关 |
| **速率限制证据位置** | `internal/middlewares/rate_limiting.go:60-94` | `internal/middlewares/rate_limiting.go:60-94` | `internal/middlewares/rate_limiting.go:60-94` |
| | | | |
...
| | | | |
| **中间件执行顺序** | 1. 速率限制 → 2. Require1FA → 3. Handler | 1. 速率限制 → 2. Require1FA → 3. Handler | 1. 速率限制 → 2. Require1FA → 3. Handler |
| **中间件证据位置** | `internal/middlewares/bridge.go:59-74` | `internal/middlewares/bridge.go:59-74` | `internal/middlewares/bridge.go:59-74` |
...
| 场景 | 恢复动作 | 等待时间步切换 |
|------|---------|--------------|
| ✅ **纯输入错误**（输错数字） | 立即重新输入正确的码即可 | **不需要**等待 |
...
| POST | `/api/secondfactor/totp` | middlewareRateLimitTOTP | 1. 速率限制检查<br>2. 已通过密码验证（Require1FA） |
| DELETE | `/api/secondfactor/totp` | middleware1FA | 需要普通 1FA 会话 |

**middlewareRateLimitTOTP 中间件执行顺序**：
1. PreMiddlewares：安全头相关（SecurityHeadersBase 等）
2. PostMiddlewares（外层到内层）：
   - **NewRateLimit**：速率限制检查 → 请求到达即计数
   - **Require1FA**：验证 1FA 会话是否有效
   - **Handler**：TOTP 验证业务逻辑
...
| 维度 | 速率限制中间件 | doMark 记录 |
|-----|--------------|------------|
| **执行时机** | 中间件层，进入 Handler 之前 | Handler 内部，验证逻辑执行中 |
| **触发条件** | 只要请求到达 endpoint 就触发 | 只有在验证失败/成功时调用 |
| **作用对象** | 基于 IP（令牌桶算法） | 基于用户/IP（监管日志） |
| **用途** | 防止暴力请求频率 | 记录审计日志、计算封禁 |
| **与结果关系** | 完全无关，只看请求是否到达 | 取决于验证成功/失败 |
| **代码位置** | `internal/middlewares/rate_limiting.go` | `internal/handlers/response.go` |
| **封禁关联** | 不直接关联封禁 | 可能触发封禁（仅 AuthType1FA） |
...
| **速率限制计数** | ✅ 已增加 | 请求已到达 endpoint，在进入 handler 前完成计数 |

**可恢复条件（统一口径）**：
- ✅ **可以立即重试**，不是必须等待时间步切换
- ✅ 不触发封禁（BanTypeNone + AuthTypeTOTP）
- ⚠️ 但重试时会再次触发速率限制计数（与上次失败原因无关）
...
| **可恢复条件** | 必须等待下一个时间步（30 秒）才能用新码通过 | 无需等待，立即重试即可通过（可能已放行） |
...
| **速率限制计数** | ✅ 都增加（请求已到达） | ✅ 都增加（请求已到达） |
| **速率限制与配置关系** | ❌ 与配置无关 | ❌ 与配置无关 |
...
```
          ...
          │                       ▼                        │
          │   ┌─────────────────────────────────────┐   │
          │   │  Require1FA：验证 1FA 会话           │   │
          │   └───────────────────┬──────────────────┘   │
          └───────────────────────┼──────────────────────┘
                                  │
                                  ▼
                    进入 Handler：TimeBasedOneTimePasswordPOST
                                  │
          ┌───────────────────────┴───────────────────────┐
...
| 速率限制 | ❌ 无 | ✅ 有（middlewareRateLimitTOTP，基于 IP） |
| 防重放检查 | ✅ 记录时间步 | ✅ 检查+记录时间步（可配置禁用） |
| 会话再生 | ❌ 不执行 | ✅ 验证成功后执行 |
| 监管封禁 | ❌ 不触发 | ❌ 不触发（仅 1FA 触发） |
| 临时密钥有效期 | ✅ 10 分钟 | ❌ 不适用 |
...
| 速率限制核心逻辑 | `internal/middlewares/rate_limiting.go` | 60-94 |
| 中间件执行顺序 | `internal/middlewares/bridge.go` | 59-74 |
...
| 路由与中间件注册 | `internal/server/handlers.go` | 299-307 |
...
**修订版本 R5**（本次更新）：
1. ✅ 关键修正：速率限制计数与 doMark **完全无关**
2. ✅ 明确：只要请求到达 endpoint，速率限制就 +1，与后续验证结果无关
3. ✅ 新增：速率限制与 doMark 对比表，澄清两个机制的独立性
4. ✅ 更新：三条路径的"速率限制计数"统一为 ✅ 增加（都到达了 endpoint）
5. ✅ 新增：速率限制证据位置、中间件执行顺序说明
6. ✅ 新增：middlewareRateLimitTOTP 中间件执行顺序详细说明
7. ✅ 统一全文档相关表述：删除所有"速率限制计数不增加"的错误说法
